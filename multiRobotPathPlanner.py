...
class MultiRobotPathPlanner(DARP):
    def __init__(self, nx, ny, notEqualPortions, initial_positions, portions,
        ...
        if not self.DARP_success:
            ...
            #print("Número de células em cada caminho:")
            #for i, num_cells in enumerate(best_case_num_paths):
                #print(f"Robô {i+1}: {num_cells} células")


            # Exibir os caminhos completos de cada robô no terminal
            #print("\nCaminhos completos dos robôs:")
            #for i, path in enumerate(best_case_returnPaths):
                #print(f"\nCaminho do robô {i+1}:")
                #for step in path:
                    #print(f"  {step}")
            ...
            self.metricas_dados = {
            "tempo_execucao": self.execution_time,
            "path_lenght_final": best_case_num_paths,
            "turns_totais": self.best_case.turns,
    ...
    def get_best_paths(self):
        """ Retorna os melhores caminhos como uma tupla """
        return tuple(self.best_case.paths)
    
    def get_best_case_num_paths(self):
        return [len(x) for x in self.best_case.paths]
...
if __name__ == '__main__':
    ...
    planner = MultiRobotPathPlanner(args.grid[0], args.grid[1], args.nep, args.in_pos, args.portions, args.obs_pos, args.vis)